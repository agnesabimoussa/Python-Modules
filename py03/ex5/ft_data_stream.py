...
def high_levels(data: list[dict], min_level: int = 10) -> Iterator[dict]:
    for event in data:
        if event['data']['level'] >= min_level:
            yield event


def treasure_events(data: list[dict]) -> Iterator[dict]:
    for event in data:
        if event['event_type'] == 'treasure':
            yield event


def level_up_events(data: list[dict]) -> Iterator[dict]:
    for event in data:
...
if __name__ == "__main__":
    ...
    for item in it:
        print(f"Event {i}: ", end="")
        print(f"Player {item['player']} ", end="")
        print(f"(level {item['data']['level']}) ", end="")
    ...
    high_count = 0
    for _ in high_levels(DATA):
        high_count += 1
    treasure_count = 0
    for _ in treasure_events(DATA):
        treasure_count += 1
    level_up_count = 0
    for _ in level_up_events(DATA):
        level_up_count += 1
